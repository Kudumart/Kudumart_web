...
export default function BookMarkedProducts() {
  ...
  const getSavedProducts = async () => {
    try {
      ...
      const productObjects = Array.isArray(products)
              ...
              {bookmarkedProducts.length > 0 ? (
                <div className={`grid grid-cols-2 md:grid-cols-4 gap-4`}>
                  {bookmarkedProducts.map((item) => (
                    <div
                      key={item.id}
                      className="bg-white shadow-lg p-1 border rounded-lg relative"
                    >
                      <Link to={`/product/${item.id}`}>
                        ...
                        <div className="p-3 w-full">
                          ...
                          <p className="text-sm font-semibold leading-loose">
                            {item.store.currency.symbol} {item.price}
                          </p>
                          <button
                            className={`absolute top-2 right-0 px-2 py-1 text-xs rounded font-meduim text-white ${
                          ...
                          <span
                            className={`absolute top-2 left-0 px-2 py-1 text-xs rounded font-meduim text-white ${
